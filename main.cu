// Field arithmetic (pairwise product of two arrays of field elements) on a GPU 
...
std::vector<uint8_t*> compute_product(std::vector<uint8_t*> a, std::vector<uint8_t*> b, uint8_t* input_m_base) {
    ...
    // Allocate memory to uint8_t array 'input_m' of size 128 x 1024 bytes which defines the modulus
